...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    protected final void rehashing() {
        /* COMPLETAR */
        int novaCap = siguientePrimo(2 * elArray.length);
        ListaConPI<EntradaHash<C, V>>[] oldArray = elArray;
        elArray = new LEGListaConPI[novaCap];
        
        for(int i = 0; i < elArray.length; i++){
            elArray[i] = new LEGListaConPI<EntradaHash<C, V>>();
        }
        talla = 0;
        for(ListaConPI<EntradaHash<C, V>> cubeta: oldArray){
    ...
    public final double desviacionTipica() {
        /* COMPLETAR */
        double res = 0;
        for(ListaConPI cubeta: elArray){
            res += (cubeta.talla() - factorCarga()) * (cubeta.talla() - factorCarga());
        }
    ...
    public final double costeMLocalizar() {
        ...
        for(ListaConPI<EntradaHash<C, V>> cubeta: elArray){
            costMitja += cubeta.talla() * (1 + cubeta.talla()) / 2;
        }
        return costMitja / talla;
    }
