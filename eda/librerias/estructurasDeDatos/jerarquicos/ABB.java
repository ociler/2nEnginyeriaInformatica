...
public class ABB<E extends Comparable<E>> {
    ...
    public ABB(E[] v) {        
        // COMPLETAR
        construirEquilibrado(v, 0, v.length - 1);
    }
    ...
    protected NodoABB<E> construirEquilibrado(E[] v, int ini, int fin) {
        // COMPLETAR  
        int meitat = (ini + fin) / 2;
        NodoABB esq = null;
        NodoABB dret = null;
        if(ini < fin){
            esq = construirEquilibrado(v, ini, meitat - 1);
            dret = construirEquilibrado(v, meitat + 1, fin);
        }
        return new NodoABB<E>(v[meitat], esq, dret);
    }
