...
public class EditorPredictivo extends ABB<String> {
    ...
    public ListaConPI<String> recuperarSucesores(String prefijo, int n) {       
        ...
        if(recuperar(prefijo) != null){res.insertar(prefijo);}
        
        String succActual = prefijo;
        while(res.talla() < n){
            succActual = sucesor(succActual);
            if(succActual != null && succActual.startsWith(prefijo)){
                res.insertar(succActual);
            }
            else{break;}
        } 
        return res;	
